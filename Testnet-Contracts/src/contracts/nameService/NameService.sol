 ...
 * 2. Register: Reveal the username and complete registration within 30 seconds
...
interface IEmailRegistery {
    function register(
        address user,
        string memory email,
        uint256 timestampUser,
        bytes memory signatureUser,
        uint256 timestampAuthority,
        bytes memory signatureAuthority
...
contract NameService {
    ...
    /// @dev Proposal system for email registry contract address changes with time delay
    AddressTypeProposal addressEmailRegistery;

    /// @dev Proposal system for phone number registry contract address changes with time delay
    ...
    /**
     * @notice Quick setup function for registries (TESTNET/CTF ONLY)
     * @dev Bypasses the time delay for immediate setup
     * @param _emailRegistry Address of the email registry contract
     * @param _phoneRegistry Address of the phone registry contract
     */
    function quickSetupRegistries(
        address _emailRegistry,
        address _phoneRegistry
    ) external onlyAdmin {
        addressEmailRegistery.current = _emailRegistry;
        addressPhoneNumberRegistery.current = _phoneRegistry;
    }

    /**
     * @notice Pre-registers a username hash to prevent front-running attacks
     * @dev Creates a temporary reservation that can be registered 30 seconds later
    ...
    function preRegistrationUsername(
        ...
        identityDetails[key] = IdentityBaseMetadata({
            owner: user,
            expireDate: block.timestamp + 30 seconds,
     ...
     * @dev Must be called after the pre-registration period (30 seconds) has reached
    ...
    /**
     * @notice Completes email registration using a pre-registration commitment
     * @dev Must be called after the pre-registration period (30 seconds) has reached
     * @param _user Address of the user completing the registration
     * @param _nonce Unique nonce to prevent replay attacks
     * @param _email The actual email being registered (revealed from hash)
     * @param _timestampUser Timestamp of the user's registration
     */
    ...
    function registrationPhoneNumber(
        address _user,
        uint256 _nonce,
        ...
        isValidPhoneNumber(_phoneNumber);

        makePay(
            _user,
            getPriceOfRegistration(_phoneNumber),
            _priorityFeeForFisher,
            _nonce_Evvm,
            _priority_Evvm,
            _signature_Evvm
    ...
    /**
     * @notice Proposes to change the email registry contract address
     * @dev Critical function that affects email registration integration
     * @param _newEmailRegisteryAddress Address of the new email registry contract
     */
    function proposeChangeEmailRegisteryAddress(
        address _newEmailRegisteryAddress
    ) public onlyAdmin {
        if (_newEmailRegisteryAddress == address(0)) {
            revert();
        }
        addressEmailRegistery.proposal = _newEmailRegisteryAddress;
        addressEmailRegistery.timeToAccept = block.timestamp + 1 days;
    }

    /**
     * @notice Cancels the pending email registry address change proposal
     * @dev Only the current admin can cancel pending proposals
     */
    function cancelChangeEmailRegisteryAddress() public onlyAdmin {
        addressEmailRegistery.proposal = address(0);
        addressEmailRegistery.timeToAccept = 0;
    }

    /**
     * @notice Executes the approved email registry address change
     * @dev Can only be called after the time delay has passed
     */
    function acceptChangeEmailRegisteryAddress() public onlyAdmin {
        if (block.timestamp < addressEmailRegistery.timeToAccept) {
            revert();
        }
        addressEmailRegistery = AddressTypeProposal({
            current: addressEmailRegistery.proposal,
            proposal: address(0),
            timeToAccept: 0
        });
    }
    ...
    /**
     * @notice Proposes to change the phone number registry contract address
     * @dev Critical function that affects phone number registration integration
     * @param _newPhoneNumberRegisteryAddress Address of the new phone number registry contract
     */
    function proposeChangePhoneNumberRegisteryAddress(
        address _newPhoneNumberRegisteryAddress
    ) public onlyAdmin {
        if (_newPhoneNumberRegisteryAddress == address(0)) {
            revert();
        }
        addressPhoneNumberRegistery.proposal = _newPhoneNumberRegisteryAddress;
        addressPhoneNumberRegistery.timeToAccept = block.timestamp + 1 days;
    }

    /**
     * @notice Cancels the pending phone number registry address change proposal
     * @dev Only the current admin can cancel pending proposals
     */
    function cancelChangePhoneNumberRegisteryAddress() public onlyAdmin {
        addressPhoneNumberRegistery.proposal = address(0);
        addressPhoneNumberRegistery.timeToAccept = 0;
    }

    /**
     * @notice Executes the approved phone number registry address change
     * @dev Can only be called after the time delay has passed
     */
    function acceptChangePhoneNumberRegisteryAddress() public onlyAdmin {
    ...
    function hashUsername(
        ...
        return keccak256(abi.encodePacked(_email, _randomNumber));
    }

        function hashPhoneNumber(
        string memory _phoneNumber,
        uint256 _randomNumber
    ) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(_phoneNumber, _randomNumber));
    }
    ...
    function isUsernameAvailable(
        ...
        if (identityDetails[_email].expireDate == 0) {
            return true;
        } else {
            return
                identityDetails[_email].expireDate + 60 days <
                block.timestamp;
        }
    }
        ...
        if (identityDetails[_phoneNumber].expireDate == 0) {
            return true;
        } else {
            return
                identityDetails[_phoneNumber].expireDate + 60 days <
                block.timestamp;
        }
    }
    ...
    /**
     * @notice Gets the current email registry contract address
     * @dev Returns the address of the email registry contract used for email registration
     * @return The current email registry contract address
     */
    function getEmailRegisteryAddress() public view returns (address) {
        return addressEmailRegistery.current;
    }

    /**
     * @notice Gets complete email registry address information including pending proposals
     * @dev Returns current email registry address, proposed address, and proposal acceptance deadline
     * @return currentEmailRegisteryAddress Current email registry contract address
     * @return proposalEmailRegisteryAddress Proposed new email registry address (if any)
     * @return timeToAcceptEmailRegisteryAddress Timestamp when proposal can be accepted
     */
    function getEmailRegisteryAddressFullDetails()
        public
        view
        returns (
            address currentEmailRegisteryAddress,
            address proposalEmailRegisteryAddress,
            uint256 timeToAcceptEmailRegisteryAddress
        )
    {
        return (
    ...
    /**
     * @notice Gets the current phone number registry contract address
     * @dev Returns the address of the phone number registry contract used for phone number registration
     * @return The current phone number registry contract address
     */
    function getPhoneNumberRegisteryAddress() public view returns (address) {
        return addressPhoneNumberRegistery.current;
    }

    /**
     * @notice Gets complete phone number registry address information including pending proposals
     * @dev Returns current phone number registry address, proposed address, and proposal acceptance deadline
     * @return currentPhoneNumberRegisteryAddress Current phone number registry contract address
     * @return proposalPhoneNumberRegisteryAddress Proposed new phone number registry address (if any)
     * @return timeToAcceptPhoneNumberRegisteryAddress Timestamp when proposal can be accepted
     */
    function getPhoneNumberRegisteryAddressFullDetails()
        public
        view
        returns (
            address currentPhoneNumberRegisteryAddress,
            address proposalPhoneNumberRegisteryAddress,
            uint256 timeToAcceptPhoneNumberRegisteryAddress
        )
    {
        return (
            addressPhoneNumberRegistery.current,
